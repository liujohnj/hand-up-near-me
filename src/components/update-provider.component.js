...
export default class UpdateProvider extends Component {
  ...
  componentDidMount() {
    axios.get('http://localhost:5000/providers/'+this.props.match.params.id)
      .then(response => {
        this.setState({
  ...
  onSubmit(e) {
    ...
    //update provider data to back end
    axios.post('http://localhost:5000/providers/update/' + this.props.match.params.id, provider)
  ...
  render() {
    ...
    <div>
      ...
      <form onSubmit={this.onSubmit}>
        ...
        <div className="form-group">
          <input type="submit" value="Update Provider" className="btn btn-primary" />
        </div>
      </form>
    </div>
    )
